...
const enemyDeathSound = new Audio("sounds/chicote.mp3");
enemyDeathSound.volume = 1;

const shootSound = new Audio("sounds/ooooo.ogg");
shootSound.volume = 1;
    ...
    for (let i = 0; i < bulletArray.length; i++) {
        ...
        for (let j = 0; j < alienArray.length; j++) {
            ...
            if (!bullet.used && alien.alive && detectCollision(bullet, alien)) {
                ...
                alienCount--;
                score += 100;
                enemyDeathSound.currentTime = 0.5;
...
function createAliens() {
    for (let c = 0; c < alienColumns; c++) {
        for (let r = 0; r < alienRows; r++) {
            alienImg.src = "./alien"+Math.floor(Math.random() * 4)+".png";
            let alien = {
                img : alienImg,
